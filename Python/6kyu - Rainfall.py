def mean(town, strng):
    ...
    Each town ends with ':'
    Each month is 3 characters
    Each month and data are split by ','
    '''
    town_data = {}
    for towns in strng.split('\n'): # seperate each mont's data
        town_name = towns[:towns.find(':')] # obtain the month's name
        monthly_data = [months if months[:months.find(':')] != town_name else months.replace(months[:months.find(':')+1],'') for months in list(towns.split(','))]
        month,data = [months[:3] for months in monthly_data], [float(months[4:]) for months in monthly_data] # split data and make values a float
        town_data[town_name] = dict(zip(month,data)) # zip into a dictionary and add dictionary to town data

#    previous solution:
#         month = []
#         data = []
#         for months in list(towns.split(',')): # split the data within the current month
#             if months[:months.find(':')] in town_name: # remove the month's name and colon from the list
#                 months = months.replace(months[:months.find(':')+1],'')
#             month.append(months[:3]) # add to month list
#             data.append(float(months[4:])) # add to data list (must be float)
#         town_data[town_name] = dict(zip(month,data)) # zip into a dictionary and add dictionary to town data
    
    try:
        mean_result = sum([town_data[town][month] for month in town_data[town]])/12 # 12 months each year - FACT!  
        return mean_result
    except KeyError:
        return -1.0
...
def variance(town, strng):
    ...
    - find the mean
    - subtract the mean from each number (the difference) and square the result
    - then work out the average from the result
    '''
    town_data = {}
    for towns in strng.split('\n'): # seperate each mont's data
        town_name = towns[:towns.find(':')] # obtain the month's name
    ...
    try:
        variance_result = sum([(town_data[town][month] - mean(town,strng))**2 for month in town_data[town]])/12
